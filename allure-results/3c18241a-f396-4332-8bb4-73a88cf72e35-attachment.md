...
- Name: features\search\search.feature.spec.js >> Search >> Product Search
- Location: .features-gen\features\search\search.feature.spec.js:6:7
...
Expected pattern: /INVALID_TITLE/
...
```ts
  ...
  9  | });
  10 | 
  11 | When('user searches for {string}', async ({}, product: string) => {
  12 | 
  13 |     console.log(product);
  14 | 
  15 | });
  16 | 
  17 | Then('search results should be displayed', async ({ page }) => {
  18 | 
> 19 |     await expect(page).toHaveTitle(/INVALID_TITLE/);
     |                        ^ Error: expect(page).toHaveTitle(expected) failed
  20 | 
  21 | });
```